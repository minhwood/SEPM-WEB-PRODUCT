...
class allDisplayFunctions 
{
    ...
    function displaySpecialProducts()
    {
        ...
        foreach ($products as $productType => $product) 
        {
            ...
            foreach($productTypes as  $product)
            {
                ...
                <div class='price' style='color:darkred'>$".($product->price - 0.1* $product->price)." (10% off) </div>";
                ...
                if($test==0)
                echo "<a href='memberSpecial.php?productName=$product->name&productType=$productType'><img class='add-cart-button ' src='img/add-cart.svg' alt='addToCartButton'></a>";
                else
                {
                    echo "<a href='yourcart.php' target='Test'><span class='glyphicon glyphicon-shopping-cart productDisplay-cart'></span></a>";
                }
	...
	function displayHomeProducts()
	{
        ...
        foreach ($products as $productType => $product) 
        {
            ...
            if($test==0)
            echo "<a href='firstpage.php?productName=$product->name&productType=$productType&page='index.php''><img class='add-cart-button ' src='img/add-cart.svg' alt='addToCartButton'></a>";
            else
            {
                echo "<a href='yourcart.php' target='Test'><span class='glyphicon glyphicon-shopping-cart productDisplay-cart'></span></a>";
            }
    ...
    function loginUsers($username,$password,$page)
    {
        $url = 'user.json'; 
        $data = file_get_contents($url); 
